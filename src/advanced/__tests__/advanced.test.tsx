...
import { CartItem, Coupon, Product } from '../../refactoring/shared/types/types';
...
describe('advanced > ', () => {
  ...
  describe('useCart > ', () => {
    ...
    test('쿠폰을 적용해야지', () => {
      ...
      act(() => {
        result.current.selectCoupon(testCoupon);
      });
    ...
    test('합계를 정확하게 계산해야 합니다', () => {
      ...
      const { result: couponResult } = renderHook(() => useCouponStore());

      act(() => {
        cartResult.current.addToCart(testProduct);
        cartResult.current.updateQuantity(testProduct.id, 2);
        couponResult.current.selectCoupon(testCoupon);
      });
