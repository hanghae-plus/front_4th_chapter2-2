...
import {
  ...
  useCartLocalStorage,
} from "../../refactoring/hooks/useCartLocalStorage";
...
describe("useCartLocalStorage >", () => {
  ...
  test("장바구니에 제품을 추가하고 localStorage에 저장해야 합니다", () => {
  ...
  test("장바구니에서 제품을 제거하고 localStorage를 업데이트해야 합니다", () => {
    const { result } = renderHook(() => useCartLocalStorage());
  ...
  test("제품 수량을 업데이트하고 localStorage를 업데이트해야 합니다", () => {
    const { result } = renderHook(() => useCartLocalStorage());
  ...
  test("쿠폰을 적용하고 localStorage에 저장해야 합니다", () => {
    const { result } = renderHook(() => useCartLocalStorage());
    ...
    const savedCoupon = JSON.parse(
      localStorage.getItem(COUPON_LOCAL_STORAGE_KEY) || "null",
    );
    expect(savedCoupon).toEqual(testCoupon);
  });

  test("합계를 정확하게 계산하고 할인을 적용해야 합니다", () => {
    const { result } = renderHook(() => useCartLocalStorage());
  ...
  test("페이지 새로고침 시 장바구니 상태가 유지되어야 합니다", () => {
    // 먼저 장바구니에 상품 추가
    const { result: initialResult } = renderHook(() => useCartLocalStorage());
    act(() => {
      initialResult.current.addToCart(testProduct);
    });

    // hook을 다시 렌더링하여 새로고침 시뮬레이션
    const { result: reloadedResult } = renderHook(() => useCartLocalStorage());
    expect(reloadedResult.current.cart).toHaveLength(1);
    expect(reloadedResult.current.cart[0]).toEqual({
      product: testProduct,
      quantity: 1,
    });
  });
